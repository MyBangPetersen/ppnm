...
public class main{
	static (double,double) plainmc(Func<vector,double> f,vector a,vector b,int N, Random RND = null){
		int dim = a.size;
		double V = 1;
	...
	//the function we are supposed to integrate
	...
	public static void Main(string[] args){
		foreach(var arg in args){
			if(arg == "Out.txt"){
				...
				WriteLine($"We are testing with 2*r*sin(theta)^2, with r from 0 to 1 and theta from 0 to 2*PI");
				WriteLine("We should then recieve PI");
				WriteLine($"res_value = {res_value} +- {res_error}");

				WriteLine("The task integral ∫0 to π  dx/π ∫0 to π  dy/π ∫0 to π  dz/π [1-cos(x)cos(y)cos(z)]^(-1)");
